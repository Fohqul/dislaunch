...
public override void activate () {
	...
	signal_list_item_factory.bind.connect (
		(_, object) => {
			...
			var label = new Gtk.Label (message.string) {
				halign = Gtk.Align.START, valign = Gtk.Align.END, wrap = true,
	...
	message_box.append (
		new Adw.Spinner () {
			height_request = 50, width_request = 50
		}
	);

	message = new Gtk.Label ("Starting") {
	...
	case "":
		if (last_status == "update_check") {
			if (release_state.version != release_state.internal.latest_version) {
				...
				checked = true;
				append_message (
					"Last attempt to check for updates failed - %d remaining".printf (
						attempts_remaining)
				);
