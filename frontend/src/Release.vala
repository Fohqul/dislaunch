...
public Release (Adw.ApplicationWindow application_window, ReleaseChannel channel) {
	...
	update_button.clicked.connect (
		(button) => channel.command (
			button.label ==
			"Update" ? "install" : "check_for_updates"
		)
	...
	// stale state
	command_line_arguments_row = new Adw.EntryRow () {
		title = "Command-Line Arguments", show_apply_button = true
	};
	command_line_arguments_row.apply.connect (
		(row) => Socket.command (
			"%s command_line_arguments %s".printf (channel.id, row.text)
		)
	);
	...
	bd_enabled_row = new Adw.ExpanderRow () {
		title = "BetterDiscord"
	};
